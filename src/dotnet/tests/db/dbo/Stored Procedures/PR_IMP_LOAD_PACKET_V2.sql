CREATE PROCEDURE [dbo].[PR_IMP_LOAD_PACKET_V2] --use model type not from PacketType, but from data.
  @ContextID int,
  @UserID int
as
begin
...
/* get mode type id from ##temp_import_csv_ */
--Get modelname and Code from ##temp_import_csv_xxxxxxx
declare @resultModel nvarchar(max)
declare @resultCode nvarchar(max)
set @sql = N'select top 1 @resModel = MODELNAME, @resCode=CODE from ' + @temp_import_csv_tablename
exec sp_executesql @sql, N'@resModel nvarchar(max) OUTPUT, @resCode nvarchar(max) OUTPUT', @resModel = @resultModel output, @resCode = @resultCode output
-- Get modeltype code from previous parameters
declare @resultMdelTypeID int
set @sql = N'select  TOP 1 @res =TYPEID from dbo.PR_MODELS where NAME=''' + @resultModel + ''' and CODE = ''' + @resultCode + ''''
exec sp_executesql @sql, N'@res int OUTPUT', @res = @resultMdelTypeID output
  ...
  select
    ...
    @impModelTypeID = @resultMdelTypeID,
