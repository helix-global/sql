CREATE procedure [dbo].[PR_OPERATION_UPD_LIST_PARAMS] @aDeviceID int
as 
SET nocount on
...
from(
	select A.ID as PARAMID,dbo.PR_DEVICE_PARAM(C.ID,A.ID) as PVALUE
	from PR_DEVICE C with (nolock)
	left join PR_MODELS B with (nolock) on B.ID = C.MODELID
	left join PR_MODELTYPE_PARAMS A with (nolock) on A.TYPEID = B.TYPEID
	where C.ID = @aDeviceID
	  and (isnull(A.USEINLIST,0) > 0  or isnull(A.USEINOPLIST,0) > 0)
	  and A.PARAMKIND = 1
) M 
