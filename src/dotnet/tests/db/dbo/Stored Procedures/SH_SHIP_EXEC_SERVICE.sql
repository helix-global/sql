CREATE procedure [dbo].[SH_SHIP_EXEC_SERVICE] @DocumentID int, @UserID int 
as
set nocount on
...
begin
   ...
   select @cou = count(*) 
    ...
    if @cou > 0
    begin
      set @errmsg = 'Only items produced in the "'+@depName+'" department can be included in this shipment request.'
      raiserror(@errmsg,15,0)
      set nocount off
      return
    end
    ...
    begin
       ...
       insert into @allowedStates (OID) values (1000085) /*shipped after srv*/
    end
    if @blockInProd <> 1
    begin
   ...
   update PR_DEVICE set S_S = 1000085 /*1000010  KB765*/
        , SHIPPED_DT = @dd
        --, SHIPPINGSTOCK = null
...
begin
   ...
   select @cou = count(*) 
     from PR_DEVICE A 
    where A.ID in (select D.DEVICEID from SH_ORDER_T D where D.SHORDERID = @DocumentID)
      and A.S_S not in (1000010,1000008,1000085)
