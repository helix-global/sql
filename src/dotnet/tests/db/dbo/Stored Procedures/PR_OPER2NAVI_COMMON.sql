--KB5279: Catched error message should be warning.
--KB5257: Additional exception diagnostics.
CREATE procedure [dbo].[PR_OPER2NAVI_COMMON] @OperID int, @aUserID int, @aDate datetime
WITH EXECUTE AS OWNER, RECOMPILE
as 
SET nocount on

declare @sn nvarchar(max)
declare @ordn nvarchar(max)
declare @ordn2 nvarchar(max)
declare @emplN nvarchar(max)
declare @devID int
declare @depID int
declare @mtID int
declare @cou int
...
declare @serviceOrd nvarchar(max)
declare @depCode nvarchar(max)
declare @rootServiceItemSN nvarchar(max) = null
declare @orderID int
declare @mdlID int
...
begin try
  insert into PR_NAVIOUT (OPERID,PARTNUMBER,PARTSN,QUANTITY,VERS,BOMID,BATCHN)
    select @OperID,dbo.PR_NAV_PN_REPLACE(B.CODE,B.REPLACEX),A.SN,isnull(A.PARTQUANTITY,1),2,A.BOMID,A.BATCHN
...
begin catch
  print '#WSQL: Warning when inserting data into [PR_NAVIOUT].';
  print '#WSQL: ' + error_message();
end catch

begin try
  insert into PR_NAVIOUT (OPERID,PARTNUMBER,PARTSN,QUANTITY,VERS,BOMID,UNITEMSTAT,REPAIRABLE,BATCHN)
    select @OperID,dbo.PR_NAV_PN_REPLACE(B.CODE,B.REPLACEX),I.SN,-isnull(I.PARTQUANTITY,1),2,I.BOMID,A.UNITEMSTAT,C.REPAIRABLE,I.BATCHN
    from PR_OPERATION_UNINSTALL A
      left join PR_OPERATION_INSTALL I on I.ID = A.INSTALLROWID
      left join PR_MODELS B with (nolock) on B.ID = I.PARTMODELID
...
begin try
  insert into PDB_BUFFER..MATERIALS (S_S,S_CR,S_CDT,PARTNUMBER,OUTDATE,SN,QUANTITY,PRODUCTIONORDER,SERVICEORDER,EMPLOYEENUMBER,PARTSN,OPERATIONID,DEPID,LOCATION,FAILED,BATCHNUMBER,ROOTSERVICEITEMSN)
    select
    1
    ,@aUserID
    ,@aDate
    ...
    ,@aDate
    ,upper(@sn)
    ,case when REPAIRABLE = 0 and S3 = 1 and DELTA = -1 then 0 else DELTA end /*если неремонтопригодный и дефектный, то ноль */
    ,@ordn2
    ,@serviceOrd
    ,@emplN
    ,PARTSN
    ...
    ,case S3 when 1 then 1 else 0 end as S3
    ...
    ,BATCHN
    ,ASDEFECTIVE
    ,(select sum(B1.QUANTITY) from PR_NAVIOUT B1 
    ...
    ,(select sum(B1.QUANTITY) from PR_NAVIOUT B1 
       where B1.OPERID = @OperID 
         ...
         and isnull(B1.ASDEFECTIVE,0) = isnull(A.ASDEFECTIVE,0)
         and B1.VERS = 2) as S2
    ,(select MAX(isnull(B3.UNITEMSTAT,0)) from PR_NAVIOUT B3 
       where B3.OPERID = @OperID 
         ...
         and B3.VERS = 2) as S3     
    ,(select MAX(isnull(B4.REPAIRABLE,0)) from PR_NAVIOUT B4 
       where B4.OPERID = @OperID 
         ...
         and B4.VERS = 2) as REPAIRABLE
     
    from 
     ( select distinct PARTNUMBER,PARTSN,BATCHN,ASDEFECTIVE from PR_NAVIOUT where OPERID = @OperID ) A
    )M
    where ISNULL(S1,0) != ISNULL(S2,0)
    )M2
end try
begin catch
  print '#WSQL: Warning when inserting data into [PDB_BUFFER]...[MATERIALS].';
  print '#WSQL: ' + error_message();
end catch

delete from PR_NAVIOUT where OPERID = @OperID and VERS = 1

update PR_NAVIOUT set VERS = 1 where OPERID = @OperID
