...
/*
25.04.2024	KB4748 (Efimov)
*/

CREATE PROCEDURE [dbo].[MSG_MANAGMENT_ABSENCE_REPORT_OTHER] @aUserID int, @aPRODDEP int, @aDELIVERYDEPID int
AS
BEGIN
...
--declare @aPRODDEP int = 4 --poland
--declare @aDELIVERYDEPID int = 4505 --IPG POLAND 
--/* TEST DATA*/

declare @REPORTWEEKDAY int = 5		-- On Fridays
declare @REPORTTIME time = '06:00'	-- Report create and send time
declare @DELIVERYTYPE int = 2302	-- Notification about the absence of top management POLAND
declare @DELIVERYDEPID int = @aDELIVERYDEPID -- not IPGL
...
if (DATEPART(dw,GETDATE()) <> @REPORTWEEKDAY)
begin
...
where PRODDEP in (@aPRODDEP) -- Departments in IPGL depending country (4 = poland)



-- переменные для хранения частей отчета (для табличной части <TR>)
declare @vacation_res as varchar(max)
declare @approve_vacation_res as varchar(max)
declare @approve_cancelation_res as varchar(max)
...
set @vacation_res = (
	...
	from 
		@VACATION V
	WHERE 
		V.PRODDEP  = @aPRODDEP						-- Country Department
...
set @res = @res + '<' + @SIZE_HEADER + ' class="header">All Departments</' + @SIZE_HEADER + '>' + CHAR(13)

if (@vacation_res is not null or @approve_vacation_res is not null or @approve_cancelation_res is not null)
	begin
		-- Absent today
		if (@vacation_res is not null)
		begin
			...
			set @res = @res + CHAR(13) +
			  ...
			  + isnull(replace( replace( @vacation_res, '&lt;', '<' ), '&gt;', '>' ) +  + CHAR(13), '')			-- Absence

			  + '</table>' + CHAR(13)
		end
	
		-- Absence requests
		if(@approve_vacation_res is not null)
		begin
			...
			set @res = @res +
			  ...
			  + replace( replace( @approve_vacation_res, '&lt;', '<' ), '&gt;', '>' ) +  + CHAR(13)
		...
		if(@approve_cancelation_res is not null)
		begin
			...
			set @res = @res +
			  ...
			  + '<col style="width:33%"><col style="width:33%"><col style="width:33%">' + CHAR(13) +
			  + '<thead><tr><th>Employee</th><th>Absence period</th><th>Link to document</th></tr></thead>'
			  + replace( replace( @approve_cancelation_res, '&lt;', '<' ), '&gt;', '>' ) +  + CHAR(13)
			  + '</table>'
		end
	end
else
...
declare @HTML varchar(MAX) = 
		...
		dbo.COM_DEP_HEADS_VACATIONS_HTML_TABLE(@nowDate, 8, @aPRODDEP) + CHAR(13) +
