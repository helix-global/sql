CREATE procedure [dbo].[PR_CHECK_PLACED_ORDERS_2NAV] @OrderID int, @UserID int, @aMode int, @guid nvarchar(50) out
...
declare @orders table (ID int not null primary key)
...
where
	ord.ID in (select ID from @orders)
union all
select
	@SessionID
	,ord.NN
...
left join COM_CUSTOMER cust on ord.CUSTOMERID = cust.ID
left join COM_DEPARTMENTS dep on ord.DEPARTMENTID = dep.ID
where ord.ID in (select ID from @orders)
  and isnull(mopt.INTOPTION,0) <> 1

update PR_PRORDER set PR_PRORDER.OUT2NAVGID = @SessionID where PR_PRORDER.ID in (select ID from @orders)
