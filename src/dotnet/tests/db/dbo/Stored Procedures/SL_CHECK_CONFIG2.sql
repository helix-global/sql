CREATE procedure [dbo].[SL_CHECK_CONFIG2] @OrderID int, @aMode int, @aUserID int 
as 
...
from SL_QUOTE2_T A 
left join SL_QUOTE2_TO B on B.OPOSID = A.ID 
...
BEGIN
   ...
   begin
     select @tmp = count(B.ID) from SL_QUOTE2_TO B 
...
select top 1 @modelErrCust = C.CODE
 from SL_QUOTE2 A
 left join SL_QUOTE2_T B on B.VNESHID = A.ID
...
select top 1 @modelErrCust = C.CODE
 from SL_QUOTE2 A
 left join SL_QUOTE2_T B on B.VNESHID = A.ID
 left join SL_QUOTE2_TO BB on BB.OPOSID = B.ID
...
left join SL_QUOTE2_T B on B.VNESHID = A.ID
left join SL_QUOTE2_TO C on C.OPOSID = B.ID
