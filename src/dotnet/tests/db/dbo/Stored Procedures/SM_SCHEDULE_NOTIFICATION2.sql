...
CREATE PROCEDURE [dbo].[SM_SCHEDULE_NOTIFICATION2] @UserID int, @WoID int, @aMode int  --Send msg with attachments
AS
BEGIN
  set nocount on

  /* KB2582 */
  declare @MbLim int = 15  -- Setup Limit in Megabytes for attachments
  ...
  /* KB2582 */
  declare @BytesLim int = @MbLim * 1024 * 1024 -- Calculated limit in bytes for compare
  declare @WoFilesSize int = isnull( (select SUM(FILESIZE) from SM_WORKORDER_FILES with (nolock) where VNESHID = @WoID group by VNESHID), 0) -- attachment size

  if @WoFilesSize > @BytesLim
    begin -- without attachments
	  print '#WWarning: Cannot add all files from Work Order to the notification, the maximum files size (' + CONVERT(varchar, @MbLim) + 'Mb) limit exceeded'
	  exec MSG_SEND_TOEMPLOYEE2 @UserID, @emplID, 'support.europe@ipgphotonics.com', @subj, @mess
    end
  else 
    begin -- with attachments
  	  declare @msTo nvarchar(1024)
	  declare @messID numeric(18,0) 
	  ...
	  if len(ltrim(rtrim(@msTo))) > 0  /*KB3109*/
	  begin
	  
		  -- make a email
		  INSERT INTO MSG_OUTGOING (S_S, GID, MSGTO, MSGSUBJ, MSGBODY, S_CDT, S_CR , MSGCC ) 
		  values (0,newid(),@msTo, @subj, @mess, getdate(), @UserID, 'support.europe@ipgphotonics.com')
		        
		  set @messID = @@identity
		  
		  -- make a attachments
		  insert into MSG_OUT_ATTACHEMENTS (GID,VNESHID,FILENAME,FILEDATE,FILESIZE,FILEBLOB)
		  select newid(),@messID, WF.FILENAME,WF.FILEDATE,WF.FILESIZE,WF.FILEBLOB
		  from SM_WORKORDER_FILES WF with (nolock) 
		  where WF.VNESHID = @WoID
