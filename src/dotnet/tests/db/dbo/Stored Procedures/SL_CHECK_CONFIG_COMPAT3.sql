CREATE procedure [dbo].[SL_CHECK_CONFIG_COMPAT3] @DocID int, @aMode int, @aUserID int 
as 
/*KB2600 проверка совместимости если включен режим совместимости 
версия для sql_quote3 который для проверки квот навижен
*/
set nocount on
...
select @CheckID = A.ID
      ...
      ,@modelsCount = (select count(distinct MODELID) from SL_QUOTE3_T J with (nolock) where J.VNESHID = A.ID) 
  ...
  */
  ...
  BEGIN
      ...
      where A.QTROWID <> @oneRowID
        and dbo.SL_ROWS_ARE_COMPATIBLE3(A.QTROWID,@oneRowID) = 1
  ...
  if @err is not null
  begin
    set @err = '#EModel '+@err+' is not compatible with other models in configuration or additional options required.'
    raiserror(@err,16,0)
  end
