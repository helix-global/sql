create PROCEDURE [dbo].[SM_CHANGESN_NOTIFICATION] @UserID int, @WoID int, @aMode int
AS
BEGIN
  set nocount on
...
/*  KB2219 */
  ...
  set @subj = 'Item of the work order '+@woNN+' has been changed. New Item: '+@model+' - '+@sn+' - '+@requestor+' '+@SCnd
  
  set @mess = @mess + '<br>'
  set @mess = @mess + 'Item of the work order '+@woNN+' has been changed. New Item: '+@model+' - '+@sn+' - '+@requestor+'<br>'
  set @mess = @mess + '<br>'
  ...
  set @mess = @mess + '<tr><td valign="top" height="32">Confirmed begin of sevice intervention:</td><td valign="top">'+isnull(dbo.COM_FORMAT_DATETIME(@schArrival,1),'NA')+'</td></tr>'
