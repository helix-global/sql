create procedure [dbo].PR_CREATE_NEXT4_backup20160802
  @MapID int,@DoneOper int,@DoneLevel int,@NextLevel int,@OrdID int,@DeviceID int,@lastUserInProgress int,@UserID int,@parentOpID int,@TrMapN int,@InQuantity int
as 
SET nocount on
    ...
    insert into @Opers (REVOPERID,OPERID,CANDO,OPEREXISTS,VISMODE,OPLEVEL,LEVELUP,CONDITION,TC_ACTION,TC_MINUTE,SCHEME_GROUP,CREATEDBYOPERID,CREATEHIDDEN)
    select A.ID,O.ID,1,0,isnull(GR.VISTYPE,0),@DoneLevel,ISNULL(F.FLOWNOWAIT,0),isnull(A.CONDITION,0),A.TC_ACTION,A.TC_MINUTE,A.SCHEME_GROUP,@DoneLevel,isnull(F.CREATEHIDDEN,0)
    ...
    */
    ...
    where LEVELUP = 1
      ...
      and not exists (select A.ID from PR_OPERATION A with (nolock) 
                    where A.DEVICEID = @DeviceID 
                      and A.ORDERID = @OrdID
                      and A.OPLEVEL in ("@Opers".OPLEVEL,@NextLevel)
                      and A.REVOPERID = "@Opers".REVOPERID
                      and isnull(A.PARENTID,0) = isnull(@parentOpID,0)
                      and isnull(A.TRMAP_N,0) = isnull(@TrMapN,0)
                      and A.S_S <> 1000023 /*анулировано*/
                      and A.CREATEDBYOPERID = @NextLevel)                      
    ...
    where VISMODE in (1,2) /* пролонгировать привязку пользователя */
      and CANDO = 1 
    ...
    insert into PR_OPERATION (GID,S_S,ORDERID,DEVICEID,OPERTYPEID,S_CDT,S_CR,REVOPERID,USERINPROGRESS,OPLEVEL,TC_ACTION,TC_MINUTE,OPERGR,MANHOUR,PARENTID,TRMAP_N,TODOTEXT,Q_IN,CREATEDBYOPERID)
    select newid(),1000032,@OrdID,@DeviceID,A.OPERID,@now,@UserID,A.REVOPERID,A.SETUSERID,A.OPLEVEL,A.TC_ACTION,A.TC_MINUTE,A.SCHEME_GROUP,A.MANHOUR,@parentOpID,@TrMapN,A.TODOTEXT,@InQuantity,A.CREATEDBYOPERID
    ...
    BEGIN
        ...
        exec PR_CREATE_NEXT4 @MapID,@SkippedOper,@DoneLevel,@NextLevel,@OrdID,@DeviceID,@lastUserInProgress,@UserID,@parentOpID,@TrMapN,@InQuantity
