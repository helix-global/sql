CREATE procedure [dbo].[PR_TIME2NAVI_IPM] @OperID int, @Mode int, @aUserID int, @aDate datetime
...
select 
  /*isnull(ltrim(rtrim(str(E.PERSONALNO))),'NA')*/
  --isnull(E.PERSONALNO,'NA')
  isnull(rtrim(ltrim(cast(E.PERSONALNO as nvarchar(20)))),'NA')
  ,A.ELAPSED_D
  ,E.QUALIFICATION
from PR_OPERATION_TIME A
left join COM_EMPLOYEE E on E.ID = A.EMPID
left join PR_OPERATION O on O.ID = A.OPERID
...
insert into PDB_BUFFER..TIMINGS (S_S,S_CR,S_CDT,OPERATIONCODE,OUTDATE,SN,QUANTITY,PRODUCTIONORDER,SERVICEORDER,EMPLOYEENUMBER,OPERATIONID,DEPID,LOCATION)
select 
   1
  ,@aUserID
  ,@aDate
