CREATE FUNCTION [dbo].[MNT_NOTIFICATION_CYCLES_LEFT_HTMLROW_EXP](@MntPlanID int,@OperID int,@EqID int, @cycles int)
...
declare @linkeqType nvarchar(200)
declare @linkremark nvarchar(max)

declare @mplan_cycles int
set @mplan_cycles = (select top 1 WORKCYCLES from MNT_PLAN with (nolock) where ID = @MntPlanID)
...
left join EQ_TYPES G with (nolock) on G.ID = B.EQTYPEID
where A.ID = @EqID
...
where A.ID in (select eq.EQID 
                    from MNT_PLAN_EQ_LINKED_EQ eq with (nolock)
                        join MNT_PLAN_EQ peq with (nolock) on eq.VNESHID=peq.ID
...
begin   
    ...
    set @linqedEqDescr = @linqedEqDescr + 
        '<td>'+isnull(@linkModelName,'NA')+'</td><td>'+isnull(@linkSN,'NA')+'</td><td>'+isnull(@linkTAGN,'NA')+'</td><td>'+isnull(@linkWP,'NA')+'</td><td>'+isnull(@linkdep,'NA')+'</td><td>'+isnull(@linkrespPerson,'NA')+'</td><td>'+isnull(@linkeqType,'NA')+'</td><td>'+isnull(@linkremark,'')+'</td></tr>'
...
/*<tr><th>Maintenance Plan</th><th>Operation Date</th><th>Equipment Model</th><th>Equipment SN</th><th>Equipment TAG Nr.</th><th>Working Place</th><th>Department</th></tr>*/

set @res = '<tr><td>'+@PlanName+'</td><td>'
/* + dbo.COM_FORMAT_DATETIME(@dd,1) */   
+ CONVERT(varchar(10), @cycles) +' work cycles from ' + CONVERT(varchar(10), @mplan_cycles)
+'</td><td>'+isnull(@ModelName,'NA')+'</td><td>'+isnull(@SN,'NA')+'</td><td>'+isnull(@TAGN,'NA')+'</td><td>'+isnull(@WP,'NA')+'</td><td>'+isnull(@dep,'NA')+'</td><td>'+isnull(@respPerson,'NA')+'</td><td>'+isnull(@eqType,'NA')+'</td><td>'+isnull(@remark,'')+'</td></tr>'
...
set @res = '<tr><td>'+@PlanName+'</td><td>'+dbo.COM_FORMAT_DATETIME(@dd,1)+'</td><td>'+isnull(@ModelName,'NA')+'</td><td>'+isnull(@SN,'NA')+'</td><td>'+isnull(@TAGN,'NA')+'</td><td>'+isnull(@WP,'NA')+'</td><td>'+isnull(@dep,'NA')+'</td></tr>'
