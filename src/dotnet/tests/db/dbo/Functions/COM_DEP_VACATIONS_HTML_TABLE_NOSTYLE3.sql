...
CREATE FUNCTION [dbo].[COM_DEP_VACATIONS_HTML_TABLE_NOSTYLE3](@startDate date, @weeks int, @DEPID int)
returns varchar(max)
as
begin
...
--	  select * from @allUserDates;
      ...
      SELECT 
		...
		'<td>' +
		 ...
		 case 
			...
			--sick leave
			when aDAYS.VacationType = 20 then '<div title="Seek Leave"  class="seek_leave"><a href="a2l:\\Link=doc.com_vacation.' + convert(nvarchar(10),aDAYS.ID) + '">&nbsp;&nbsp;</a></div>' 

			--short absence
			when aDAYS.VacationType = 30 and aDAYS.S_S in (1000141,2130051) then '<div title="Short Absence approved" class="short_approved"><a href="a2l:\\Link=doc.com_vacation.' + convert(nvarchar(10),aDAYS.ID) + '">&#x25CF;</a></div>' 
			when aDAYS.VacationType = 30 and aDAYS.S_S not in (1000141,2130051) then '<div title="Short Absence unapproved" class="short_not_approved"><a href="a2l:\\Link=doc.com_vacation.' + convert(nvarchar(10),aDAYS.ID) + '">&#x25CF;</a></div>' 
			...
			when aDAYS.VacationType in (15,50,60,70,980,100) then
																case 
																	...
																	then
																		case
																			when  isnull(aDAYS.PeriodType,1) = 1 then '<div class="fullday_approved">'
																			when  isnull(aDAYS.PeriodType,1) = 2 then '<div class="forenoon_approved">'
																			when  isnull(aDAYS.PeriodType,1) = 3 then '<div class="afternoon_approved">'
																			else '<div>'
																		end
																	else			--not approved
																		case
																			when  isnull(aDAYS.PeriodType,1) = 1 then '<div class="fullday_not_approved">'
																			when  isnull(aDAYS.PeriodType,1) = 2 then '<div class="forenoon_not_approved">'
																			when  isnull(aDAYS.PeriodType,1) = 3 then '<div class="afternoon_not_approved">'
																			else '<div>'
																		end
																end
																...
																+ 
																case --types
																	when isnull(aDAYS.VacationType, -1) = 15 then '<div title="Unpaid Leave"><a href="a2l:\\Link=doc.com_vacation.' + convert(nvarchar(10),aDAYS.ID) + '">U</a></div>'
																	when isnull(aDAYS.VacationType, -1) = 50 then '<div title="Business Trip"><a href="a2l:\\Link=doc.com_vacation.' + convert(nvarchar(10),aDAYS.ID) + '">B</a></div>'
																	when isnull(aDAYS.VacationType, -1) = 60 then '<div title="Training"><a href="a2l:\\Link=doc.com_vacation.' + convert(nvarchar(10),aDAYS.ID) + '">T</a></div>'
																	when isnull(aDAYS.VacationType, -1) = 70 then '<div title="Special Leave"><a href="a2l:\\Link=doc.com_vacation.' + convert(nvarchar(10),aDAYS.ID) + '">S</a></div>'
																	when isnull(aDAYS.VacationType, -1) = 80 then '<div title="Internal Appointment"><a href="a2l:\\Link=doc.com_vacation.' + convert(nvarchar(10),aDAYS.ID) + '">I</a></div>'
																	when isnull(aDAYS.VacationType, -1) = 90 then '<div title="Parental Leave"><a href="a2l:\\Link=doc.com_vacation.' + convert(nvarchar(10),aDAYS.ID) + '">P</a></div>'
																	when isnull(aDAYS.VacationType, -1) = 100 then '<div title="Child Care"><a href="a2l:\\Link=doc.com_vacation.' + convert(nvarchar(10),aDAYS.ID) + '">C</a></div>'
																	else ''
																end
      ...
      set @weekTimeLine = @weekTimeLine +  case when @Week = @weeks then '</tr></table></td>' else  '</tr></table></td><td><table><tr>' end
      set @Week = @Week + 1;
      set @WeekFirstDay = dateadd(week,1,@WeekFirstDay)
    END;
