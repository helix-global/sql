CREATE FUNCTION [dbo].[PR_WORKTIME2BETWEEN]
(
	@WorkTimeID int, @Now datetime, @aDBeg datetime, @aDEnd datetime
)
RETURNS int
AS
BEGIN
...
select @wtID = ISNULL(A.PERSONALWT,B.ID), @Calendar = ISNULL(B2.CALENDAR,B.CALENDAR)
   ...
   where A.ID = @emplID;
      ...
      with
      times0 as
        (select case when A.DBEG < @aDBeg then @aDBeg else A.DBEG end as dbeg,
                case when ISNULL(A.DEND,@Now) > @aDEnd then @aDEnd else ISNULL(A.DEND,@Now) end as dend
                ,A.ID
                ,coalesce(B.PREP_RESULT,B.Q_IN,1) as QTY
