create function [dbo].[CS_FP_PROD_REPORT_KB2181](@aUserID int, @modelGroupID int, @dBeg datetime, @dEnd datetime, @revIds nvarchar(MAX), @aMode int)
returns @res table (YYYY int, PROD_QTY decimal(18,2), GOOD_QTY decimal(18,2), BAD_QTY decimal(18,2), UNDEF_QTY decimal(18,2))
as 
begin

  /*
  KB2181 добавлкн вариант по кварталам: @aMode = 1
  при этом в YYYY пишется <год><квартал>  например 1 квартал 2021:  20211
  */
...
/* по маске YYMMDD-#-#-# */
  ...
  select ID
   , case when @aMode = 1 then year(DD)*10 + (case when month(DD) in (1,2,3) then 1 
                                                   when month(DD) in (4,5,6) then 2 
                                                   when month(DD) in (7,8,9) then 3
                                                   when month(DD) in (10,11,12) then 4 end) 
                          else year(DD) end
