CREATE function [dbo].[PR_WORKTIME5]
(
...
/* 
по сравнению с PR_WORKTIME4 убирает периоды отсутствий 
*/
...
union  select distinct dend from @times
      ...
      taskperiods as (
         select ID
               , dbo.COM_WORK_MINUTS6(spoint,epoint,@wtID,@Calendar,@emplID)*QTY/sum(QTY) over (partition by spoint, epoint) as busytime
           from periods
          inner join @times t
