CREATE function [dbo].[SL_GETOPTIONS3] (@aModelID int,@aExOptions nvarchar(max))
...
from SL_OPTIONS A
    join SL_MODEL_OPTIONS B on A.ID=B.OPTIONID and B.MODELID = @aModelID and isnull(B.PREDEFINEDOPT,0) = 0
where A.TYPEID = @mtid
