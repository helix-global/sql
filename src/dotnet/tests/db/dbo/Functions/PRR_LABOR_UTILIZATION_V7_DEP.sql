CREATE FUNCTION [dbo].[PRR_LABOR_UTILIZATION_V7_DEP](@DepID int, @aMonths int, @OnlyMonth date, @IncludeCurrentMonth int ,@mode int)
RETURNS 
@res TABLE 
(
     EMPLID int    
...
BEGIN
  ...
  left join COM_EMPL_PERIODS P with(nolock) on P.EMPLID=E.ID and P.DEPID in (select ID from @departments)
	...
	--and exists (select * from dbo.COM_EMPLOYEE_IN_DEP_RANGE(E.ID, @DepID, @from, @now, 0))
  ...
  where exists (select * from dbo.COM_EMPLOYEE_IN_DEP_RANGE(A.ID, @DepID, B.DBEG, B.DEND_NEXT, 0))
    ...
    */
	...
	begin
		...
		update @res set H_AVAILABLE_PLAN = dbo.COM_WORK_MINUTS_BY_DEP3(@DepID, DBEG,case when DEND < @now2 then DEND else @now2 end,CALENDARID,EMPLID,0,22)
