...
CREATE function [dbo].[COM_TURNS_TOMORROW] (@dd datetime, @whID int, @emplID int)
returns table 
as 
/* вспомогательная функция выводит смены завтра относительно @dd*/
/* KB3535 */
    ...
    with 
     ...
     as (
		 ...
		 select dateadd(day,+1,cast(@dd as date))  --KB3535
     )
	 ,works
	  as (
