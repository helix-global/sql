CREATE function [dbo].[SL_ROWS_ARE_COMPATIBLE3](@aRow1 int, @aRow2 int)
returns int
as
begin
   ...
   */
   ...
   select @Model1 = A.MODELID from SL_QUOTE3_T A with (nolock) where A.ID = @aRow1
   
   select @Model2 = A.MODELID 
         ,@Model2MT = B.TYPEID
     from SL_QUOTE3_T A with (nolock) 
   ...
   if exists (select A.MODELID 
				 ...
				 where A.MODELID = @Model1
				   ...
				   and exists (select J.ID 
				                 from SL_QUOTE3_TO J with (nolock) 
