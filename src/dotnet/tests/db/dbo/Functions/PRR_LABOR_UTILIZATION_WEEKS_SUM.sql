CREATE FUNCTION [dbo].[PRR_LABOR_UTILIZATION_WEEKS_SUM](@DepID int, @aMonth int, @aYear int)
RETURNS 
@res TABLE 
(
     DEPID int    
    ...
    PRIMARY KEY (DEPID, WW, YEAR)
)
AS
BEGIN
    
    insert into @res (DEPID,YEAR,WW,DBEG,DEND,E_INCLUDED,H_AVAILABLE,H_VACATIONS,H_INOPERATIONS,H_AVAILABLE_RANDD,H_AVAILABLE_PRODSUPP,H_INOPERATIONS_CONST)
    select @DepID
           ,case when @aMonth=1 and A.WW = 52 then A.YEAR -1 else A.YEAR end as YEAR
           ,A.WW
           ,A.DBEG
           ,A.DEND
           ,sum(A.E_INCLUDED)
    ...
    from dbo.PRR_LABOR_UTILIZATION_WEEKS(@DepID, @aMonth, null, @aYear) A
    left join COM_EMPLOYEE B with (nolock) on B.ID = A.EMPLID
    group by A.YEAR,A.WW,A.DBEG,A.DEND
