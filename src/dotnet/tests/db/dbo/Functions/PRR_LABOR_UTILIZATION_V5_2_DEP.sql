/* KB3321 */
CREATE FUNCTION [dbo].[PRR_LABOR_UTILIZATION_V5_2_DEP](@DepID int, @aMonths int, @OnlyMonth date, @IncludeCurrentMonth int ,@mode int)
RETURNS 
@res TABLE 
(
     EMPLID int    
...
BEGIN
  ...
  left join COM_EMPLOYEE E on E.ID=TT.EMPID
    WHERE TT.EMPID in (select ID from @empl)
      ...
      and A.COMPLETED_DT >= @from
      and A.COMPLETED_DT <= @now 
      and TT.DBEG>=@from-90 --MG_08.11.22
  ...
  where exists (select * from dbo.COM_EMPLOYEE_IN_DEP_RANGE(A.ID, @DepID, B.DBEG, B.DEND_NEXT, 0))
    ...
    */
	...
	begin
		...
		update @res set H_AVAILABLE_PLAN = dbo.COM_WORK_MINUTS_BY_DEP3(@DepID, DBEG,case when DEND < @now2 then DEND else @now2 end,CALENDARID,EMPLID,0,2)
