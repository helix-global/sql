CREATE function [dbo].[COM_WEEK_PERIOD3] (@dBeg datetime, @dEnd datetime)
...
begin
    ...
    begin
       ...
       insert into @res (WEEKID,YY,WW,DDATE,DDATE_PLUS1
                        ,WEEK_MONDAY,WEEK_SUNDAY
                        ,WEEK_COMPLETED
                        ,WEEK_NOW)    
       values (year(@dd)*100+datepart(iso_week,@dd),year(@dd),datepart(iso_week,@dd),@dd,DATEADD(week, 1, @dd)
                        ,dateadd(day,1-@dayOfWeek,@dd),dateadd(day,7-@dayOfWeek,@dd)
