CREATE function [dbo].[COM_WORK_MINUTS_BY_DEP3] (@depId int, @dBeg datetime, @dEnd datetime, @calendar int, @emplID int, @includeChildDeps bit, @mode int)
returns decimal(12,2)
as 
begin

    /* версия 3 с параметром @mode, который передается в COM_WORKPERIODS_BY_DEP3 для харкода смены алгоритма по отделу FP */

    declare @workM decimal(14,2);
    
    with 
    worksall2
      as (select A.DBEG as dbeg, A.DEND as dend from dbo.COM_WORKPERIODS_BY_DEP4(@depId,@dBeg,@dEnd,@calendar,@emplID,@includeChildDeps,@mode) A where A.DBEG < @dEnd)
    ,added_work  
        as (select DATEADD(mi, DATEDIFF(mi, 0, B.DBEG), 0) as dbeg  /* без секунд и ms */
                  ,DATEADD(mi, DATEDIFF(mi, 0, B.DEND), 0) as dend  /* без секунд и ms */
                   from COM_ADDED_WORKTIME B with (nolock) 
                   where B.EMPLID = @emplID 
                     and B.DBEG < @dEnd 
                     and B.DEND > @dBeg
                     and dbo.COM_EMPLOYEE_IN_DEP3(B.EMPLID,@depID,cast(B.DBEG as date),@includeChildDeps) = 1  /*KB4221*/)
