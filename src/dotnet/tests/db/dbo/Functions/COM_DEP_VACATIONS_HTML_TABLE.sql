...
CREATE FUNCTION [dbo].[COM_DEP_VACATIONS_HTML_TABLE](@startDate date, @weeks int, @DEPID int)
returns varchar(max)
as
begin
	...
	/* TEST PARAMETERS */
	--declare @weeks int = 8
	--declare @DELIVERYTYPE int = 9999 -- Test Delivery Type
	--declare @DEPID int =  184 -- 1 prod, 2 supp dep
	--declare @startDate date = '20221212' --getdate() --'20221212'
	/* TEST PARAMETERS */
	...
	select V.EMPLID, E.[NAME] as EMPLNAME, V.DBEG, isnull(V.DEND, V.DBEG), V.VACATIONTYPE, V.PERIODTYPE, V.S_S, V.ID
	...
	where 
		(
			...
			and 
			V.DBEG <= DATEADD(week,@weeks-1,@nowDATE)) /* начнуться не позже отображаемых недель */
		or 
			(V.DBEG <= @nowDATE  /* начнуться раньше сегодня */
				and 
			isnull(V.DEND, V.DBEG)  >= @nowDATE) /* и кончаться после сегодня */
		)
	and V.S_S in (1000140, 1000141)--123661
	and E.DEPID = @DEPID
	order by E.[NAME]
	...
	BEGIN
	   ...
	   from 
		...
		--DECLARE @tableHead varchar(max) = '<tr><th><a href="a2l:\\Link=view.com_vacation_timeline_hd">' + @DEPNAME + '</a></th><th>CW' + convert(varchar,1 + (case when @weekShift < 52 then @weekshift else @weekshift-52 end) )  + '</th>'
		DECLARE @tableHead varchar(max) = '<tr><th><a href="a2l:\\Link=view.com_vacation_timeline">' + @DEPNAME + '</a></th><th>CW' + convert(varchar,1 + (case when @weekShift < 52 then @weekshift else @weekshift-52 end) )  + '</th>'
				...
				@weekTimeLine = ISNULL(@weekTimeLine ,'') + 
					...
					case when aDAYS.DAYN = 0	--нет absence 
						then 
							...
							case when aDAYS.VacationType = 30 -- короткое отсутсвие
								then 
									case when aDAYS.S_S = 1000141 then '<span class="approved">' + @shortAbsence30 + '</span>' else '<a class="pdblink" href="a2l:\\Link=doc.com_vacation.'+ convert(varchar,aDAYS.ID) +'"><span class="needapprove">' + @shortAbsence30 + '</span></a>' end
								else 
									case 
										when aDAYS.PeriodType = 1 then case when aDAYS.S_S = 1000141 then '<span class="approved">' + @fullday1 + '</span>' else '<a class="pdblink" href="a2l:\\Link=doc.com_vacation.'+ convert(varchar,aDAYS.ID) +'"><span class="needapprove">' + @fullday1 + '</span></a>' end
										when aDAYS.PeriodType = 2 then case when aDAYS.S_S = 1000141 then '<span class="approved">' + @forenoon2 + '</span>' else '<a class="pdblink" href="a2l:\\Link=doc.com_vacation.'+ convert(varchar,aDAYS.ID) +'"><span class="needapprove">' + @forenoon2 + '</span></a>' end
										when aDAYS.PeriodType = 3 then case when aDAYS.S_S = 1000141 then '<span class="approved">' + @afternoon3 + '</span>' else '<a class="pdblink" href="a2l:\\Link=doc.com_vacation.'+ convert(varchar,aDAYS.ID) +'"><span class="needapprove">' + @afternoon3 + '</span></a>' end
										when aDAYS.PeriodType = 4 then case when aDAYS.S_S = 1000141 then '<span class="approved">' + @fullday4 + '</span>' else '<a class="pdblink" href="a2l:\\Link=doc.com_vacation.'+ convert(varchar,aDAYS.ID) +'"><span class="needapprove">' + @fullday4 + '</span></a>' end
										else case when aDAYS.S_S = 1000141 then '<span class="approved">' + @fullday1 + '</span>' else '<a class="pdblink" href="a2l:\\Link=doc.com_vacation.'+ convert(varchar,aDAYS.ID) +'"><span class="needapprove">' + @fullday1 + '</span></a>' end 
									end
							end
					end
	...
	+ @table
	+ '<br />' + CHAR(13)
	+ '<div class="legend">Approved&#x2000;&#x2000;&#x2000;&#x2000;&#x2000;&#x2000;:&#x2000;<span class="approved">&#x2588;</span> - Full day, <span class="approved">&#x2580;</span> - Forenoon, <span class="approved">&#x2584;</span> - Afternoon, <span class="approved">&#x25CF;</span> - Short absence</div> ' + CHAR(13) +
	+ '<div class="legend">Need to Approve:&#x2000;<span class="needapprove">&#x2588;</span> - Full day, <span class="needapprove">&#x2580;</span> - Forenoon, <span class="needapprove">&#x2584;</span> - Afternoon, <span class="needapprove">&#x25CF;</span> - Short absence</div>' + CHAR(13) +
	+ '
	</body>
	</html>
	'
