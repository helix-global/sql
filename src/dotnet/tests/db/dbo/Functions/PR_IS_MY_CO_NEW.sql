CREATE function [dbo].[PR_IS_MY_CO_NEW](@UserID int,@OnDate datetime)  
...
begin  
  ...
  where A.OPERTYPEID in (select K.OPERTYPEID from PR_OPERATIONS_RAW_BYUSER K with (nolock, noexpand) where K.USERID = @UserID) 
    ...
    and isnull(G.VISTYPE,0) not in (1,3)
  ...
  where A.DEVICEID is null 
    /*and O.MNT_PLANID is not null  */ /*KB625*/  /* maintenance */
    and A.ORDERID is null
    and B.OPERGRID in (select F.ID from @Groups F where F.DEPID = G.DEPARTMENTID )
    and isnull(G.VISTYPE,0) not in (1,3)
