CREATE function [dbo].[COM_WORK_MINUTS6_test] (@dBeg datetime, @dEnd datetime, @whID int, @calendar int, @emplID int)
...
/* !!! для тестов перед изменением COM_WORK_MINUTS6 */
    ...
    with 
	worksall2
	  as (select A.DBEG as dbeg, A.DEND as dend from dbo.COM_WORKPERIODS(dateadd(hour,-12,@dBeg),@dEnd,@calendar,@whID,@emplID) A)
