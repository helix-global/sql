...
CREATE FUNCTION [dbo].[PRR_LABOR_UTILIZATION_V7](@DepID int, @aMonths int, @OnlyMonth date, @IncludeCurrentMonth int ,@mode int)
RETURNS 
@res TABLE 
(
     EMPLID int    
...
BEGIN
  ...
  from (
    select A.EMPLID,C.DD,sum(C.MINUTES) as MINUTES
  ...
  where exists (select * from dbo.COM_EMPLOYEE_IN_DEP_RANGE(A.ID, @DepID, B.DBEG, B.DEND_NEXT, 1))
    ...
    */
    ...
    /*KB4018*/  
    update @res 
    set H_AVAILABLE_RANDD = case when isnull(KB4248TYPE,0) = 100 then H_AVAILABLE_PLAN - isnull(H_VACATIONS,0) else OPERS_100 end
    ,H_AVAILABLE_PRODSUPPORT = case when isnull(KB4248TYPE,0) = 101 then H_AVAILABLE_PLAN - isnull(H_VACATIONS,0) else OPERS_101 end
    where AFTER_CHANGE = 1      
    /*KB4018*/  
    update @res 
    set H_AVAILABLE = 0
