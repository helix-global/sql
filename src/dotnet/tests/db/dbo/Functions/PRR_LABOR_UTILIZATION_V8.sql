CREATE FUNCTION [dbo].[PRR_LABOR_UTILIZATION_V8](@DepID int, @aMonths int, @OnlyMonth date, @IncludeCurrentMonth int ,@mode int)

/* KB5180:2025.01.22 : {Maksim Efimov} отнимаем больничные (сокращаем AVAILABLE_PLAN на больничные), так как отпуски вычтены из Vacations при подсчете выше */
...
(
    ...
    ,KB4248TYPE int           -- 100% на non-production или production support
...
BEGIN
    ...
    /*
    KB1674 @mode = 1 отключает фильтр на наличие заказа в операции
    */
  ...
  --where ID <> 219 /* ВРЕМЕННО! УБРАТЬ!!!*/

    declare @empl table (ID int primary key (ID))
    insert into @empl (ID)
  select distinct E.ID
  from COM_EMPLOYEE E with(nolock)
  left join COM_EMPL_PERIODS P with(nolock) on P.EMPLID=E.ID and P.DEPID in (select ID from @departments)
  ...
  left join COM_EMPLOYEE E on E.ID=TT.EMPID
    WHERE TT.EMPID in (select ID from @empl)
      and (A.ORDERID is not null or (@mode = 1 and A.EQID is null))
      and A.S_S in (1000013,1000019,1000038/*failure.processed*/)
      and A.COMPLETED_DT >= @from
      and A.COMPLETED_DT <= @now 
      --KB4500 and TT.DBEG>=@from-90 --MG_08.11.22
	  and isnull(EP.DEPID,E.DEPID) in (select ID from @departments)
  ...
  select EMPLID,year(DD),month(DD),sum(MINUTES)
  from (
    select A.EMPLID,C.DD,sum(C.WORKMINUTES/*KB4676*/) as MINUTES
    from COM_VACATION A with (nolock)
    outer apply dbo.COM_VACATION_MINUTES_BYDAYS2_KB4724(A.ID) C
  ...
  left join COM_EMPLOYEE E on E.ID=A.EMPLID
    where A.EMPLID in (select ID from @empl)
      ...
      and dbo.COM_VACATION_DAY_OVERRIDE(A.ID,A.EMPLID,C.DD) = 0 /*KB4465,KB4520*/
      and A.DBEG <= @now
    ...
    group by A.EMPLID,C.DD
   )M 
   group by EMPLID,year(DD),month(DD)
  ...
  where exists (select * from dbo.COM_EMPLOYEE_IN_DEP_RANGE(A.ID, @DepID, B.DBEG, B.DEND_NEXT, 1))
    ...
    */
	...
	if dbo.DEF_SYS_CONST_INT('com_adp_integration',-1)=1	/*условие для построения отчета на основе данных из ADP для USA*/	
	begin
		set @now2 = dateadd(day,-1,@now2) --чтобы COM_WORK_DAYS сработала правильно
		update @res set H_AVAILABLE_PLAN = dbo.COM_WORK_DAYS( DBEG,case when DEND < @now2 then DEND else @now2 end,isnull(CALENDARID,1)) * 8 * 60
	end
	else if dbo.PRR_ISDEP_KB4664(@DepID) = 1
	begin
		update @res set H_AVAILABLE_PLAN = (select SUM(H.WORKMINUTES) 
		                                      from COM_IMPORTED_WORKTIME H with(nolock) 
		                                     where H.EMPLID = "@res".EMPLID 
		                                       and H.WORKDAY >= "@res".DBEG 
		                                       and H.WORKDAY < "@res".DEND)
	end
	else
		update @res set H_AVAILABLE_PLAN = dbo.COM_WORK_MINUTS_BY_DEP3(@DepID, DBEG,case when DEND < @now2 then DEND else @now2 end,CALENDARID,EMPLID,1,22)
    ...
    */
    ...
    declare @chDate date  /*дата перехода на новый алгоритм*/
    select top 1 @chDate = FF.SDD from PRR_LUR_KB4018DATE FF with(nolock) where FF.STYPE = 1
    set @chDate = isnull(@chDate,'40001212')
    
    update @res set AFTER_CHANGE = 1
    where cast(DBEG as date) >= @chDate    
    
    /*по таблице в комментарии к KB4018 есть только 2 варианта после даты перехода: либо 0 либо 100%*/
    update @res set PARTINPRODUCTION = 100
    where PARTINPRODUCTION > 0
      and AFTER_CHANGE = 1
    
    /*
    update @res set OPERS_100 = dbo.PRR_OPERS_TIMEKB4018(EMPLID,YEAR,MONTH,100)
                   ,OPERS_101 = dbo.PRR_OPERS_TIMEKB4018(EMPLID,YEAR,MONTH,101)
                   ,KB4248TYPE = dbo.PRR_IS100PERCENT_KB4248(EMPLID,YEAR,MONTH)
    where AFTER_CHANGE = 1

    update @res set H_AVAILABLE_PLAN = H_AVAILABLE_PLAN - isnull(OPERS_100,0) - isnull(OPERS_101,0)
    where AFTER_CHANGE = 1
      and isnull(KB4248TYPE,0) not in (100,101)
      */
    update @res set OPERS_100 = (select sum(M.TT) 
                                   from (
									 select dbo.PRR_OPERS_TIMEKB4018_DATE(EMPLID,J.DDATE,100,@DepID) as TT
									   from dbo.COM_DAYS_IN_MONTHS(dbo.COM_ENCODE_DATE("@res".YEAR,"@res".MONTH,1),null) J
									   where J.DDATE < @now2/*KB4663*/
									) M )  
					,OPERS_101 = (select sum(M.TT) 
                                   from (
									 select dbo.PRR_OPERS_TIMEKB4018_DATE(EMPLID,J.DDATE,101,@DepID) as TT
									   from dbo.COM_DAYS_IN_MONTHS(dbo.COM_ENCODE_DATE("@res".YEAR,"@res".MONTH,1),null) J
									   where J.DDATE < @now2/*KB4663*/
									) M )  									
    where AFTER_CHANGE = 1
    ...
    where ISRANDD = 1 /*Yes*/
      and isnull(AFTER_CHANGE,0) <> 1 /*KB4018*/
    ...
    /*KB4018*/  
    update @res 
    set H_AVAILABLE_RANDD = OPERS_100/*case when isnull(KB4248TYPE,0) = 100 then H_AVAILABLE_PLAN - isnull(H_VACATIONS,0) else OPERS_100 end*/
    ,H_AVAILABLE_PRODSUPPORT = OPERS_101/*case when isnull(KB4248TYPE,0) = 101 then H_AVAILABLE_PLAN - isnull(H_VACATIONS,0) else OPERS_101 end*/
    where AFTER_CHANGE = 1      
    /*KB4018*/  
    update @res 
    set H_AVAILABLE = 0
    where AFTER_CHANGE = 1      
      and PARTINPRODUCTION = 0
