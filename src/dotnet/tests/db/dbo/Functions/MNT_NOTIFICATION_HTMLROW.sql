CREATE FUNCTION [dbo].[MNT_NOTIFICATION_HTMLROW](@MntPlanID int,@EqID int,@dNext datetime)
...
declare @eqTypeName nvarchar(200)
...
declare @linkTypeName nvarchar(200)
...
begin	
	...
	set @linqedEqDescr = @linqedEqDescr + 
		'<td>'+isnull(@linkTypeName,'NA')+'</td><td>'+isnull(@linkModelName,'NA')+'</td><td>'+isnull(@linkSN,'NA')+'</td><td>'+isnull(@linkTAGN,'NA')+'</td><td>'+isnull(@linkWP,'NA')+'</td><td>'+isnull(@linkdep,'NA')+'</td><td>'+isnull(@linkrespPerson,'NA')+'</td><td>'+isnull(@linkstateName,'NA')+'</td></tr>'
...
/*<tr><th>Maintenance Plan</th><th>Next Execution Date</th><th>Equipment Type</th><th>Equipment Model</th><th>Equipment SN</th><th>Equipment TAG Nr.</th><th>Working Place</th><th>Department</th></tr>*/

set @res = '<tr><td>'+@PlanName+'</td><td>'+ isnull(dbo.COM_FORMAT_DATETIME(@dNext,1),'')+'</td><td>'+isnull(@eqTypeName,'NA')+'</td><td>'+isnull(@ModelName,'NA')+'</td><td>'+isnull(@SN,'NA')+'</td><td>'+isnull(@TAGN,'NA')+'</td><td>'+isnull(@WP,'NA')+'</td><td>'+isnull(@dep,'NA')+'</td><td>'+isnull(@respPerson,'NA')+'</td><td>'+isnull(@stateName,'NA')+'</td></tr>' 
