CREATE function [dbo].[CS_HPLA_RP_BYSTAGES](@mtID int, @UserID int, @aMode int, @date date, @avgMonthPeriod int)
...
begin
  ...
  */
	...
	select A.STAGEID
	      ,A.ORDMONTH
		  ,SUM(A.REQTIME) 
	from dbo.CS_HPLA_RP_ORDERED_WITHSTAGES(@mtID,@UserID,0,@nowD,@avgMonthPeriod) A
