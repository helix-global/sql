CREATE function [dbo].[PR_DEVICE_BOM_1_MODELS] (@DeviceID int, @BomID int)
returns @res table (BOMID int,PARTMODELID int,PARTONLYREVID int,PARTMODELFROM int,BOMIDMODELSCOUNT int,OPTQTY int, OPTION1 int)
as 
begin
...
select G.OPTID,G.BOMID,G.QUANTITY from PR_DEVICE_OPT G where G.DEVICEID = @DeviceID and (G.BOMID is null or G.BOMID = @BomID)

/* 1 из ревизии с опциями */
insert into @res (BOMID,PARTMODELID,PARTMODELFROM,OPTION1)
select distinct A.BOMID,A.PARTMODELID,1,A.ONLYOPTION
...
/* 2 из типа */  
insert into @res (BOMID,PARTMODELID,PARTMODELFROM,OPTION1)
select distinct A.BOMITEM ,A.MODELID ,2,A.OPTIONID
...
insert into @res (BOMID,PARTMODELID,PARTMODELFROM)
select distinct A.BOMID,A.PARTMODELID,3
from PR_REV_BOM2 A with (nolock) 
where A.REVID = @RevID
...
/* 4 из типа без опций */  
insert into @res (BOMID,PARTMODELID,PARTMODELFROM)
select distinct A.BOMITEM ,A.MODELID ,4
from PR_MODELTYPE_OPTIONS_BOM A  with (nolock) 
left join PR_MODELTYPE_COMMON AA  with (nolock) on AA.ID = A.TYPEID
where AA.MTID = @MtID
