...
--KB5263:2025-07-07: Initial Update.
CREATE function [dbo].[CS_DELIVERY_RELIABILITY_MG](@ModelGroupID int, @UserID int, @DBeg datetime,@DEnd datetime, @AddDays int)
returns @OutT table ([DD] date,[V_PLUS] int,[V_MINUS] int)
begin
  declare @MGDepID int
  select
    @MGDepID = [a].[DEPID]
  from [dbo].[PR_MODEL_GROUP] [a] with(nolock)
  where [a].[ID]=@ModelGroupID
  ...
  insert into @tmp ([DEVICEID],[SO_ID],[CDD],[SHREQID])
    ...
    where [mdl].[MODELGROUPID]=@ModelGroupID
      and [mdl].[DEPID] = @MGDepID
      and [sup].[CDD] >= @DBeg
      and [sup].[CDD] < @DEnd
