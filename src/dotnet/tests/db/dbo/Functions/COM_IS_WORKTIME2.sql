CREATE function [dbo].[COM_IS_WORKTIME2](@aDT datetime, @aEmplID int)
returns int as 
begin

	--отличается от COM_IS_WORKTIME2 тем, что учитывает историю изменения графиков сотрудника  SH 15.05.2020
   
   if dbo.COM_IS_VACATION_TIME(@aDT, @aEmplID) = 1
      return -2
   
   if exists  (select A.ID from COM_ADDED_WORKTIME A with (nolock) where A.EMPLID = @aEmplID and @aDT >= A.DBEG and @aDT <= A.DEND)
      return 1
     
   declare @wtID int
   declare @Calendar int
   
   set @wtID = dbo.COM_EMPLOYEE_WORKTIME_BY_DATE(@aEmplID,@aDT)
