create FUNCTION [dbo].[PRR_LABOR_UTILIZATION_V7_DEPS_SUB](@RequestID int, @IncludeCurrentMonth int ,@mode int)
RETURNS 
...
OPEN db_cursor  
	...
	BEGIN  
		...
		from [dbo].[PRR_LABOR_UTILIZATION_V7_SUM_DEP](@DEPID, @MONTH, @YEAR, @IncludeCurrentMonth, @mode) T -- высчитываем правильно по формуле числитель и знаменатель для (отдел-месяц-год)
