CREATE function [dbo].[PR_IS_MY_CO_AJOIN](@UserID int,@OnDate datetime)  
...
begin  
  ...
  where A.COMPLETED_DT is null
    ...
    and A.USERINPROGRESS <> @UserID
    and B.OPERGRID in (select F.GROUPID from PR_EMPL_TO_OPERGR F with (nolock) 
                        where F.EMPLOYEEID = @emplID and F.DEPID = B.DEPID and isnull(F.DBEG,'19900101') <= @OnDate and isnull(F.DEND,'40000101') >= @OnDate)
    and isnull(B.AJOIN,0) = 1
