create FUNCTION [dbo].[WEB_FIND_OPERATION_2CANCELCOMPLETED_MT](@UserID int, @aMTName nvarchar(300), @aCode nvarchar(16), @aSN nvarchar(50), @aOrder nvarchar(50), @aOperationCode nvarchar(50))
RETURNS int
AS
BEGIN
  ...
  from PR_OPERATION A with (nolock, index(IX_PR_OPERATION_1)) 
  ...
  where A.DEVICEID in (select ID from @items)
    ...
    and A.S_S in (1000013)  /*completed*/
