...
CREATE FUNCTION [dbo].[PR_PRODUCTION_PLAN_DATE_HISTORY_TEST]
(
        @DepID INT
       ,@dBeg DATETIME
       ,@dEnd DATETIME
...
RETURNS @RES TABLE (
             ...
             ,DEVICECOUNT int
             ,NONORM int
             ,EMPL_MH decimal(10,3)
             ,DEVICE_MH decimal(10,3)
             ,TR_PERCENT decimal(10,3)
             ,WEEK_PERCENT decimal(10,3)
             ,WEEK_EMPL_MH decimal(10,3)
             ,HOLIDAY int
)
AS 
BEGIN
    ...
    declare @dev table (ID int,EXPDATE datetime,WEEKN int,MAPID int, REVID int,ERR int,MANH decimal(10,3), COMPLETED INT , DEV_QTY INT PRIMARY KEY(ID, WEEKN, EXPDATE))
    
	/*заполнение списка изделий*/
    insert into @dev (ID,EXPDATE,MAPID,REVID,ERR, COMPLETED, WEEKN, DEV_QTY)
     select A.ID
