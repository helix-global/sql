CREATE function [dbo].[COM_WORK_MINUTS4] (@dBeg datetime, @dEnd datetime, @whID int, @calendar int, @emplID int, @aMode int)
...
begin
    ...
    with 
	worksall2
	  as (select A.DBEG as dbeg, A.DEND as dend from dbo.COM_WORKPERIODS2(dateadd(hour,-12,@dBeg)/*KB3804*/,dateadd(hour,12,@dEnd)/*KB4148*/,@calendar,@whID,@emplID) A where A.DBEG < @dEnd)
	,added_work  
	...
	,result
	  as (
	      select A.dbeg
	            ,A.dend
	            ,case @aMode when 1 then dbo.COM_DURATION_NOTVACATION(@emplID,@whID,A.dbeg,A.dend)
	            else DATEDIFF(mi,A.dbeg,A.dend) 
	            end as duration_s
	        from result0 A
	        where A.dbeg < A.dend 
	     )
