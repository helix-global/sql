CREATE FUNCTION [dbo].[PRR_LABOR_UTILIZATION_V4_PERIOD](@DepID int, @beginDate datetime, @endDate datetime)
RETURNS 
@res TABLE 
(
	 EMPLID int    
...
BEGIN
  ...
  update @res set H_AVAILABLE_PLAN = dbo.COM_WORK_MINUTS_BY_DEP(@DepID, DBEG,case when DEND < @now then DEND else @now end,WTID,CALENDARID,EMPLID,1)
