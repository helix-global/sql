CREATE function [dbo].[COM_WORK_MINUTS_BY_DEP2] (@depId int, @dBeg datetime, @dEnd datetime, @calendar int, @emplID int, @includeChildDeps bit)
...
begin
    ...
    with 
    worksall2
      as (select A.DBEG as dbeg, A.DEND as dend from dbo.COM_WORKPERIODS_BY_DEP2(@depId,@dBeg,@dEnd,@calendar,@emplID,@includeChildDeps) A where A.DBEG < @dEnd)
    ,added_work  
