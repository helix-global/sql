CREATE function [dbo].[PR_DEVICE_PARAM_FILES] (@aDeviceID int,@aParamID int)
returns @res table (ID int,FILENAME nvarchar(255),FILEDATE datetime, FILESIZE int, FILEBLOB image, FSOURCE int, FILEPREVIEW image)
as 
begin
  ...
  begin
    ...
    insert into @res (ID,FILENAME,FILEDATE,FILESIZE,FILEBLOB,FSOURCE,FILEPREVIEW) 
    select A.ID,A.FILENAME,A.FILEDATE,A.FILESIZE,A.FILEBLOB,1,A.FILEPREVIEW
    from SW_TOOL_VER_FILES A with (nolock)
    where A.VERID = @swVerID
      and A.STID is null
      
    insert into @res (ID,FILENAME,FILEDATE,FILESIZE,FILEBLOB,FSOURCE,FILEPREVIEW) 
    select A.ID,BB.FILENAME,BB.FILEDATE,BB.FILESIZE,A.FILEBLOB,1,BB.FILEPREVIEW
    from SW_TOOL_VER_FILES A with (nolock)
  ...
  begin
    ...
    insert into @res (ID,FILENAME,FILEDATE,FILESIZE,FILEBLOB,FSOURCE,FILEPREVIEW) 
    select top 1 A.ID,A.FILENAME,A.FILEDATE,A.FILESIZE,A.FILEBLOB,2,FILEPREVIEW
  ...
  begin
      ...
      insert into @res (ID,FILENAME,FILEDATE,FILESIZE,FILEBLOB,FSOURCE,FILEPREVIEW) 
      select top 1 A.ID,A.FILENAME,A.FILEDATE,A.FILESIZE,A.FILEBLOB,3,FILEPREVIEW
