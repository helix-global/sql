CREATE function [dbo].[PR_OP_TIME_IN_DAY](@OpTimeID int, @aDate datetime, @now datetime)
returns int
as
begin
...
select @wtID = ISNULL(A.PERSONALWT,B.ID), @Calendar = ISNULL(B2.CALENDAR,B.CALENDAR)
   ...
   where A.ID = @emplID;
      ...
      taskperiods as (
         select ID
               , dbo.COM_WORK_MINUTS(spoint,epoint,@wtID,@Calendar,@emplID)/COUNT(*) over (partition by spoint, epoint) as busytime
           from periods
          inner join times t
