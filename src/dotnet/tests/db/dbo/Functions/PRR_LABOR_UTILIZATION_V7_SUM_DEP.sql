create FUNCTION [dbo].[PRR_LABOR_UTILIZATION_V7_SUM_DEP](@DepID int, @aMonth int, @aYear int, @IncludeCurrentMonth int, @mode int)
RETURNS 
@res TABLE (
	ID int,
	[YEAR] int,
...
select	@DepID ID
    ...
    from dbo.PRR_LABOR_UTILIZATION_V7_DEP(@DepID, null, @aDate, @IncludeCurrentMonth,@mode) A
