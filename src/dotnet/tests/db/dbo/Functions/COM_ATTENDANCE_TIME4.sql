CREATE function [dbo].[COM_ATTENDANCE_TIME4](@aUserID int,@aEmplID int,@aDay datetime)
returns decimal(16,3) as 
begin

/* v.4 как v3 но не только по рабочим дням, а плюс дням в кторых есть переработка KB3740
...
*/
   ...
   select @res = dbo.COM_WORK_MINUTS6 (@dd, dateadd(day,1,@dd), @wtID, @Calendar, @emplID) 
