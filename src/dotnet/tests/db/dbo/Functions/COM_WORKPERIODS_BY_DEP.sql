CREATE function [dbo].[COM_WORKPERIODS_BY_DEP] (@depId int, @dBeg datetime, @dEnd datetime, @calendar int, @whID int, @emplID int, @includeChildDeps bit)
returns table 
as 
/* в отличие от COM_WORKPERIODS2 учитывает периоды работы сотрудника в COM_EMPLOYEE и в COM_EMPL_PERIODS */
    ...
    with 
	 empldays
	  as (select cast(DD as datetime) as dt from dbo.COM_EMPLOYED_DAYS_BY_DEP(@depId,@emplID,@dBeg,@dEnd,@includeChildDeps))  
	,workdays
