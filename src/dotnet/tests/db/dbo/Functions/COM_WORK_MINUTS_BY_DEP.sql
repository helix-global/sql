CREATE function [dbo].[COM_WORK_MINUTS_BY_DEP] (@depId int, @dBeg datetime, @dEnd datetime, @whID int, @calendar int, @emplID int, @includeChildDeps bit)
...
begin
    ...
    with 
	worksall2
	  as (select A.DBEG as dbeg, A.DEND as dend from dbo.COM_WORKPERIODS_BY_DEP(@depId,@dBeg,@dEnd,@calendar,@whID,@emplID,@includeChildDeps) A where A.DBEG < @dEnd)
	,added_work  
