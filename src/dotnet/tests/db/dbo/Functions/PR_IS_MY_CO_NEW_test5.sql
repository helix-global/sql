...
CREATE function [dbo].[PR_IS_MY_CO_NEW_test5](@UserID int,@OnDate datetime)  
...
begin  
  ...
  where A.OPERTYPEID in (select K.OPERTYPEID from PR_OPERATIONS_RAW_BYUSER K with (nolock, noexpand) where K.USERID = @UserID) 
   ...
   /*24.01.2018 optimized maintenance operations */
   
  insert into @res (ID)
  select A.ID 
  from PR_OPERATION_NOCMPL A with (nolock, noexpand)
  left join PR_OPERATION O with (nolock) on O.ID = A.ID
  left join PR_OPERATIONS B with (nolock) on B.ID = A.OPERTYPEID
  left join PR_OPERATIONS_GR G with (nolock) on G.ID = B.OPERGRID
  where A.DEVICEID is null and O.MNT_PLANID is not null /* maintenance */
    and B.OPERGRID in (select F.ID from @Groups F /*where F.DEPID = G.DEPARTMENTID 27.12.2018 непонятно зачем это условие */ )
    and isnull(G.VISTYPE,0) <> 1
