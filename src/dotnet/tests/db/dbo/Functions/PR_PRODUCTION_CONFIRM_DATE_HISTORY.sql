...
--KB4124:2024-02-08:Initial
CREATE FUNCTION [dbo].[PR_PRODUCTION_CONFIRM_DATE_HISTORY]
(
        @DepID INT
       ,@dBeg DATETIME
       ,@dEnd DATETIME
...
BEGIN
    ...
    while @dd < @dEnd
    begin
       insert into @RES (CDD,WEEKN,DAY_OFWEEK) 
           values (@dd
                   ,datepart(isowk,@dd)
    ...
    update @RES set HOLIDAY = 1 where dbo.COM_IS_WORKDAY(CDD,1) = 0
   

    declare @dev table (ID int,CDD datetime,WEEKN int,MAPID int, REVID int,ERR int,MANH decimal(10,4), COMPLETED INT , DEV_QTY INT PRIMARY KEY(ID, WEEKN, CDD))
    
    /*заполнение списка изделий*/
    insert into @dev (ID,CDD,WEEKN, MAPID,REVID,ERR, COMPLETED,  DEV_QTY)
     select A.ID
           ,ISNULL(B.CDD,/*C.COMPLETED_DT*/C.DD)
           ,datepart(isowk,ISNULL(B.CDD,C.DD))
           ,A.MAPID
           ,A.REVID
           ,0
           ,CASE
     ...
     where
       ...
       and ISNULL(B.CDD,/*C.COMPLETED_DT*/C.DD) >= @dBeg
       and ISNULL(B.CDD,/*C.COMPLETED_DT*/C.DD) < @dEnd

     UNION
     
     SELECT
        -1 * ROW_NUMBER() OVER (ORDER BY ORD.CDD) AS ID
        ,ORD.CDD
        ,datepart(isowk,ORD.CDD)
        ,REV.MAPID
        ,REV.ID
        ,0
        ,0
    ...
    update @RES set DEVICECOUNT = (select SUM(DEV_QTY) from @dev where CDD = "@RES".CDD  )
    ...
    UPDATE @RES 
        SET EMPL_MH = (SELECT COUNT(ID)*8*60 FROM dbo.COM_EPLOYEES_IN_PRODUCTION(@DepID, "@RES".CDD) )

    update @RES set DEVICE_MH = (select sum(MANH) from @dev where CDD = "@RES".CDD  )

    update @RES set TR_PERCENT = (DEVICE_MH / EMPL_MH) * 100 where EMPL_MH > 0

    update @RES set NONORM = (select nullif(SUM(ERR),0) from @dev where CDD = "@RES".CDD )
...
-- SELECT * FROM [PR_PRODUCTION_PLAN_DATE_HISTORY](151, '20160301', '20160331', 0)