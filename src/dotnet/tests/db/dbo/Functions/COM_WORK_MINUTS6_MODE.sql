CREATE function [dbo].[COM_WORK_MINUTS6_MODE] (@dBeg datetime, @dEnd datetime, @whID int, @calendar int, @emplID int, @mode int, @DepID int)
...
/* отличается от COM_WORK_MINUTS3 исключением времени по утвержденным периодам отсутствия */
    ...
    with 
	worksall2
	  as (select A.DBEG as dbeg, A.DEND as dend from dbo.COM_WORKPERIODS_MODE(dateadd(hour,-12,@dBeg)/*KB3804*/,dateadd(hour,12,@dEnd)/*KB4148*/,@calendar,@whID,@emplID,@mode,@DepID) A)
