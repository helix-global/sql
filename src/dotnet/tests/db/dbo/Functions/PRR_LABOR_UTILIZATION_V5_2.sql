CREATE FUNCTION [dbo].[PRR_LABOR_UTILIZATION_V5_2](@DepID int, @aMonths int, @OnlyMonth date, @IncludeCurrentMonth int ,@mode int)
RETURNS 
@res TABLE 
(
     EMPLID int    
  ...
  where exists (select * from dbo.COM_EMPLOYEE_IN_DEP_RANGE(A.ID, @DepID, B.DBEG, B.DEND_NEXT, 1))
    ...
    */
	...
	begin
		...
		update @res set H_AVAILABLE_PLAN = dbo.COM_WORK_MINUTS_BY_DEP3(@DepID, DBEG,case when DEND < @now2 then DEND else @now2 end,CALENDARID,EMPLID,1,2)
