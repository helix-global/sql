CREATE function [dbo].[PR_IS_MY_CO_NEW2](@UserID int,@OnDate datetime)  
...
begin  
  ...
  where D.S_S in (1000008,1000011,1000029,1000078) /*in production, in service, pending production, failed*/
    ...
    and not exists (select H.ID from PR_OPERATION_INPROGRESS H with (nolock, noexpand) 
                     where H.DEVICEID = A.DEVICEID and H.ORDERID = A.ORDERID and H.OPERGR = A.OPERGR and H.USERINPROGRESS <> @UserID)
  ...
  where A.DEVICEID is null
    ...
    /*and A.MNT_PLANID is not null */ /*KB625*/ /* Maintenance plan */
