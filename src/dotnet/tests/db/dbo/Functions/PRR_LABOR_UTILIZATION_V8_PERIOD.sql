CREATE FUNCTION [dbo].[PRR_LABOR_UTILIZATION_V8_PERIOD](@DepID int, @beginDate datetime, @endDate datetime)
RETURNS 
@res TABLE 
(
     EMPLID int    
...
BEGIN
	...
	declare @from datetime = @beginDate
	declare @now datetime = @endDate
 ...
 declare @mode int = 0
  ...
  insert into @vacations (EMPLID,YY,MM,RES)
    select A.EMPLID,year(C.DD),month(C.DD),sum(C.WORKMINUTES/*KB4676*/)
  ...
  left join COM_EMPLOYEE E on E.ID=A.EMPLID
    ...
    select A.ID
          ...
          ,C.CONSTADDED
    from @empl A 
    cross join dbo.COM_MONTH_PERIOD(@from,@now) B
  ...
  where exists (select * from dbo.COM_EMPLOYEE_IN_DEP_RANGE(A.ID, @DepID, B.DBEG, B.DEND_NEXT, 1))
    ...
    */
	...
	begin
		...
		update @res set H_AVAILABLE_PLAN = dbo.COM_WORK_MINUTS_BY_DEP3(@DepID, DBEG,case when DEND < @now then DEND else @now end,CALENDARID,EMPLID,1,22)
    ...
    */
    ...
    /*
    update @res set OPERS_100 = dbo.PRR_OPERS_TIMEKB4018_PERIOD(EMPLID,@from,@now,100)
                   ,OPERS_101 = dbo.PRR_OPERS_TIMEKB4018_PERIOD(EMPLID,@from,@now,101)
   ...
   update @res set OPERS_100 = (select sum(M.TT) 
                                   from (
									 select dbo.PRR_OPERS_TIMEKB4018_DATE(EMPLID,J.DDATE,100,@DepID) as TT
									   from dbo.COM_DAYS_IN_MONTHS(@from,@now) J
									   where J.DDATE >= @from
									     and J.DDATE < @now
									) M )  
    ...
    /*KB4018*/  
    update @res 
    set H_AVAILABLE_RANDD = OPERS_100
    ,H_AVAILABLE_PRODSUPPORT = OPERS_101
    where AFTER_CHANGE = 1      
    /*KB4018*/  
    update @res 
    set H_AVAILABLE = 0
