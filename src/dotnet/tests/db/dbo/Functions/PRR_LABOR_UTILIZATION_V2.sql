CREATE FUNCTION [dbo].[PRR_LABOR_UTILIZATION_V2](@DepID int, @aMonths int, @OnlyMonth date)
RETURNS 
@res TABLE 
(
	 EMPLID int    
...
BEGIN
	...
	set @from = dateadd(month,@aMonths,@now)
	...
	select A.ID,B.YY,B.MM,B.DBEG,B.DEND_NEXT,C.RES,D.RES,E.ISRANDD,E.PRODSUPPORT,dbo.PRR_PART_IN_PROD(A.ID,B.YY,B.MM)/*15.10.2018 isnull(E.PARTINPRODUCTION,100)*/,ISNULL(E.PERSONALWT,BB.ID), ISNULL(BB2.CALENDAR,BB.CALENDAR), C.CONSTADDED
	from @empl A 
	cross join dbo.COM_MONTH_PERIOD(@from,dateadd(month,-1,@now)) B
	...
	*/
    ...
    update @res set H_AVAILABLE_PLAN = dbo.COM_WORK_MINUTS5(DBEG,DEND,WTID,CALENDARID,EMPLID)
