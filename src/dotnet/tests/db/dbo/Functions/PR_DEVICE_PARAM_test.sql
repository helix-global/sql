create function [dbo].[PR_DEVICE_PARAM_test](@DeviceID int, @ParamID int)
...
begin
  ...
  begin
	  ...
	  from PR_OPERATION_PARAMS A with (nolock)
	  left join PR_OPERATION B with (nolock) on B.ID = A.OPERID
	  where A.OPERID in (select N.OPERID from PR_PARENT_OPERATION N where N.DEVICEID = @DeviceID and isnull(N.DONTUSEPARAMETERS,0) <> 1)
		and A.PARAMID = @ParamID
		and B.S_S in (1000013,1000019,1000116)
	  order by B.ID desc
