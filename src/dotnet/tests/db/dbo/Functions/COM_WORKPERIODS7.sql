CREATE function [dbo].[COM_WORKPERIODS7] (@dBeg datetime, @dEnd datetime, @calendar int, @whID int, @emplID int, @mode int)
returns table 
as 

	/*то-же что и COM_WORKPERIODS5, но @mode = 1 - включать автосозданные переработки */
	    ...
	    as (select DATEADD(mi, DATEDIFF(mi, 0, B.DBEG), 0) as dbeg  /* без секунд и ms */
	              ,DATEADD(mi, DATEDIFF(mi, 0, B.DEND), 0) as dend  /* без секунд и ms */
	               ...
	               where B.EMPLID = @emplID 
	                 and B.DBEG < @dEnd and B.DEND > @dBeg 
	                 and (isnull(B.AUTOADDEDTIME,0) not in (1,2) or @mode = 1))
