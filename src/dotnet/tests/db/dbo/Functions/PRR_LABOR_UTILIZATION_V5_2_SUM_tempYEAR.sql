create FUNCTION [dbo].[PRR_LABOR_UTILIZATION_V5_2_SUM_tempYEAR](@DepID int, @aMonths int, @IncludeCurrentMonth int, @mode int)
RETURNS 
@res TABLE 
(
     DEPID int    
...
BEGIN
    ...
    */
    ...
    from dbo.PRR_LABOR_UTILIZATION_V5_2_tempYEAR(@DepID, @aMonths, null, @IncludeCurrentMonth,@mode) A
    ...
    (
      select sum(ST.ELAPSED)
        ...
        left join PR_PRORDER O with (nolock) on O.ID = DEV.ORDERID
        left join PR_MODELTYPE MT with (nolock) on MT.ID = MDL.TYPEID
      cross apply dbo.PR_DEVICE_PRODSUPPORT_TIME_POSTED(DEV.ID) ST
        where O.DEPARTMENTID in (select ID from dbo.PRR_CHILD_DEPID(@DepID,1))
          AND DEV.ORDERID IS NOT NULL
          and DEV.COMPLETED_DT > "@res".DBEG
          and DEV.COMPLETED_DT < "@res".DEND
