    ...
    /* Token.Whitespace     */ State.TopLevel,       State.TopLevel,      State.Error,       State.StartContent,    State.Content,    State.PostB64Cont,    State.PostB64Attr,   State.AfterRootEle,   State.Attribute,      State.SpecialAttr,   State.Error,        State.RootLevelAttr,      State.RootLevelSpecAttr,  State.PostB64RootAttr,   State.AfterRootLevelAttr, State.Error, /* Token.Whitespace     */
        ...
        public override void WriteDocType(String name,String pubid,String sysid,String subset)
            ...
            try
                ...
                SqlXmlConvert.VerifyQName(name, ExceptionType.XmlException);

                if (conformanceLevel == ConformanceLevel.Fragment)
                    {
                    throw new InvalidOperationException(Xml_DtdNotAllowedInFragment);
                    }
                ...
                if (dtdWritten)
                    {
                    currentState = State.Error;
                    throw new InvalidOperationException(Xml_DtdAlreadyWritten);
                    }
                ...
                if (checkCharacters)
                    ...
                    if (pubid != null)
                        ...
                        if ((i = xmlCharType.IsPublicId(pubid)) >= 0)
                            {
                            throw new ArgumentException(String.Format(Xml_InvalidCharacter, XmlExceptions.BuildCharExceptionArgs(pubid, i)), "pubid");
                            }
                        }
                    if (sysid != null)
                        {
                        if ((i = xmlCharType.IsOnlyCharData(sysid)) >= 0)
                            {
                            throw new ArgumentException(String.Format(Xml_InvalidCharacter, XmlExceptions.BuildCharExceptionArgs(sysid, i)), "sysid");
                            }
                        }
                    if (subset != null)
                        {
                        if ((i = xmlCharType.IsOnlyCharData(subset)) >= 0)
                            {
                            throw new ArgumentException(String.Format(Xml_InvalidCharacter, XmlExceptions.BuildCharExceptionArgs(subset, i)), "subset");
                            }
                        }
                    }
        ...
        public override void WriteStartElement(String prefix,String localName,String ns)
            ...
            try
                ...
                if (localName == null || localName.Length == 0)
                    {
                    throw new ArgumentException(Xml_EmptyLocalName);
                    }
                    ...
                    switch (specAttr)
                        ...
                        case SpecialAttribute.XmlSpace:
                            ...
                            if (value == "default")
                                ...
                                {
                                throw new ArgumentException(String.Format(Xml_InvalidXmlSpace, value));
                                }
            ...
            try
                ...
                if (name.Length == 3 && String.Compare(name, "xml", StringComparison.OrdinalIgnoreCase) == 0)
                    ...
                    if (currentState != State.Start)
                        {
                        throw new ArgumentException(conformanceLevel == ConformanceLevel.Document ? Xml_DupXmlDecl : Xml_CannotWriteXmlDecl);
                        }
        ...
        public override void WriteSurrogateCharEntity(Char lowChar,Char highChar)
            ...
            try
                ...
                if (!Char.IsSurrogatePair(highChar, lowChar))
                    {
                    throw XmlExceptions.CreateInvalidSurrogatePairException(lowChar, highChar);
                    }
                ...
                if (ns != null && ns.Length != 0)
                    ...
                    if (prefix == null)
                        ...
                        if (currentState != State.Attribute)
                            {
                            throw new ArgumentException(String.Format(Xml_UndefNamespace, ns));
                            }
                        prefix = GeneratePrefix();
                        PushNamespaceImplicit(prefix, ns);
                        }
        ...
        public override void WriteBinHex(Byte[] buffer,Int32 index,Int32 count)
            ...
            if (IsClosedOrErrorState)
                {
                throw new InvalidOperationException(Xml_ClosedOrError);
                }
            try
                {
                AdvanceState(Token.Text);
        ...
        private void WriteStartDocumentImpl(XmlStandalone standalone)
            ...
            try
                ...
                if (conformanceLevel == ConformanceLevel.Auto)
                    ...
                    {
                    throw new InvalidOperationException(Xml_CannotStartDocumentOnFragment);
                    }
        ...
        private void PushNamespaceImplicit(String prefix,String ns)
            ...
            if (existingNsIndex != -1)
                ...
                if (existingNsIndex > elemScopeStack[elemTop].prevNSTop)
                    ...
                    if (nsStack[existingNsIndex].namespaceUri != ns)
                        {
                        throw new XmlException(String.Format(Xml_RedefinePrefix,prefix, nsStack[existingNsIndex].namespaceUri, ns));
                        }
            ...
            // No existing declaration found in the namespace stack
                ...
                if ((ns == XmlReservedNs.NsXml && prefix != "xml") ||
                    ...
                    {
                    throw new ArgumentException(String.Format(Xml_NamespaceDeclXmlXmlns, prefix));
                    }
        ...
        private void ThrowInvalidStateTransition(Token token,State currentState)
            ...
            switch (currentState)
                ...
                case State.Start:
                    if (conformanceLevel == ConformanceLevel.Document)
                        {
                        throw new InvalidOperationException(wrongTokenMessage + ' ' + Xml_ConformanceLevelFragment);
                        }
                    break;
                }
