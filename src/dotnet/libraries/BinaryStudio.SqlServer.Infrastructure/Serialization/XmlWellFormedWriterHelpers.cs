...
namespace BinaryStudio.SqlServer.Infrastructure
    {
    internal partial class XmlWellFormedWriter
        {
        //
        // Private types
        //
        ...
        partial class AttributeValueCache
            ...
            internal void Trim()
                ...
                if (singleStringValue != null)
                    {
                    singleStringValue = SqlXmlConvert.TrimString(singleStringValue);
                    return;
                    }

                // trim the string in StringBuilder
                string valBefore = stringValue.ToString();
                string valAfter = SqlXmlConvert.TrimString(valBefore);
                ...
                while (i == firstItem && i <= lastItem)
                    ...
                    switch (item.type)
                        ...
                        case ItemType.Raw:
                        case ItemType.ValueString:
                            item.data = SqlXmlConvert.TrimStringStart((string)item.data);
                ...
                while (i == lastItem && i >= firstItem)
                    ...
                    switch (item.type)
                        ...
                        case ItemType.Raw:
                        case ItemType.ValueString:
                            item.data = SqlXmlConvert.TrimStringEnd((string)item.data);
                            if (((string)item.data).Length == 0)
                                {
